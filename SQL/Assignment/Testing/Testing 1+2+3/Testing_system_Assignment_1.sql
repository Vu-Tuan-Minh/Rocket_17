...
insert into `Group` values ("3","PxJ","3","2019/9/9");
...
insert into GroupAccount values ("4","2","2019/8/11");
insert into GroupAccount values ("5","1","2019/9/10");
...
select max(length(FullName)),AccountID,Email,Username,FullName,DepartmentID,PositionID,CreateDate from `ACCOUNT`;

-- Question 5: Lấy ra thông tin account có full name dài nhất và thuộc phòng ban có id 
--  = 3
select vmax(length(FullName)),AccountID,Email,Username,FullName,DepartmentID,PositionID,CreateDate from `ACCOUNT` where DepartmentID = 3;
...
-- select * from `account`where DepartmentID IN ("1");
